...
void usertrap(void)
{
  ...
  /*由于mmap使用懒加载 映射 虚拟地址->磁盘文件地址 ，所以在读取或写入相应虚拟地址时，会存在地址未映射的情况。
    这时需要分配一个物理页面，将磁盘中的文件读到该物理页，将虚拟地址映射到该物理页（页表中）。
  */
  else if (r_scause() == 13 || r_scause() == 15)
  {
    ...
    else
    {
      ...
      if (vma)
      {
        ...
        {
          //从文件中读取PGSIZE的内容到新分配的物理页中，使用readi()函数
          memset((void *)mem, 0, PGSIZE);
          ilock(vma->file->ip);
          readi(vma->file->ip, 0, mem, offset, PGSIZE);
          iunlock(vma->file->ip);
          //文件内容已经成功复制到物理页面，接下来建立va->pa的页表项吧
          int flag = PTE_U;
