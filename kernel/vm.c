...
//为内核页表增加一个新的映射，// 仅在启动时使用。不刷新 TLB 不 启用分页。
...
//为页表pagetable size/PGSIZE 个映射 ，从va 映射到 pa，一共映射sizeB。va 和 size 可能不是页面对齐的。成功则返回0
...
//为第一个进程将 用户初始化代码initcode 加载到pagetable的地址0中。sz必须小于一页。
...
//分配PTEs和物理内存 以满足 进程从oldz -> newsz的需求。返回新大小或错误时返回0。
...
// 递归释放页表页。 所有叶子映射（第三级页表，映射到可读、可写、可执行的内存）必须已经被删除。
